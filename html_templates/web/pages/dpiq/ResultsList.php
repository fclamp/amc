...
if (count($GLOBALS['HTTP_POST_VARS']) > 0)
	$req =& $GLOBALS['HTTP_POST_VARS'];
else
	$req =& $GLOBALS['HTTP_GET_VARS'];


if (preg_match("/^(\w+)\s+(\w.*)$/", $req['col_TaxGenus'], $matches))
{
        $req['col_TaxGenus'] = $matches[1];
        $req['col_TaxSpecies'] = $matches[2];
}
else
{
        $req['col_TaxSpecies|TaxGenus'] = $req['col_TaxGenus'];
        $req['col_TaxGenus'] = "";
}

require_once('../../objects/dpiq/ResultsLists.php');
$resultlist = new DpiqStandardResultsList;
