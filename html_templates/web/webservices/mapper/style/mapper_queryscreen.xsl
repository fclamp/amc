...
<xsl:template match='/'>
	...
	<head>
		...
		<script type="text/javascript" src="./mapper/style/scripts/kemapperquery.js"></script>
	</head>
	<body>
...
<div class='centred'>
	<table>
		...
		<xsl:choose>
			...
			<xsl:otherwise>	
				<input type='hidden' name='scatter' id='scatterTrue' value='true'  />
			</xsl:otherwise>	
		</xsl:choose>	
		<tr class='bordered'>
			<td class='bordered'>
			...
			<td class='bordered' style='text-align: left;'>
				By <select name='sortby'>
					<xsl:for-each select='//queryableFields/field'>
						<xsl:choose>
							<xsl:when test="./@displayName='IRN'"/>
							<xsl:when test="./@displayName='Latitude'"/>
							<xsl:when test="./@displayName='Longitude'"/>
							<xsl:when test="./@displayName='description'"/>
							<xsl:otherwise>
							<option>
		...
		<tr class='bordered'>
			<td class='bordered'>Map Style/Projection</td>
			<td class='bordered' style='text-align: left;'>
				<select name='mapfile'>
					<xsl:for-each select='//suggestedParameters/projections/availableProjection'>
						<option>
							<xsl:attribute name='value'>
								<xsl:value-of select='./mapfile'/>
							</xsl:attribute>
		...
		<input type='hidden' name='mapfile' value='world.map'/>
		...
		<tr>
			...
			<table id='potential'>	
				...
				<tr style='text-align: left'>
					<td class='potential' style='text-align: left'>
