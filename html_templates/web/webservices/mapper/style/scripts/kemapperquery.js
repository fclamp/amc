...
{
	var sourceDiv = element;
	var sourceName = element.getAttribute("srcName");
	var cb = document.getElementById("chk_" + sourceName);
	var currentClass = sourceDiv.getAttribute("class");
	var sourceHolder = document.getElementById('actual');


	sourceDiv.setAttribute("class","actual");
	if (currentClass == 'actual')
	{
		sourceHolder = document.getElementById('potential');
		sourceDiv.setAttribute("class","potential");
		cb.checked = false;
		cb.setAttribute('defaultChecked', ''); //IE6 bug workaround
	}
	else
	{
		cb.checked = true;
		cb.setAttribute('defaultChecked', 'defaultChecked'); //IE6 bug workaround
	}

	sourceHolder.appendChild(sourceDiv);
}
...
var behaviours =  {
	...
	'.clear' : function(element) {
		element.onclick = function(){
			...
			for (var i = 0; i < inputs.length; i++)
			{
				if (inputs[i].className == 'textInput')
				{
					inputs[i].value = "";
				}
			}
