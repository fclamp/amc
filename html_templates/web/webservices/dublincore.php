...
**
** Provide a Dublin Core query interface.  Designed for KE Web Portal.
** 	See Chris Dance for details.
** 
** example: http://server/web/webservices/dublincore.php?title=fish&creator=smith
**
...
$WEBSERVICE_NAME		= "dublincorequery";
...
// Valid DC Fields
$DUBLIN_CORE_FIELDS = array(	'title', 
				'creator', 
				'subjectkeywords', 
				'description',
				'publisher',
...
main()
{
	...
	*/
	$interfaces = ParseInterfaceDefinitions($GLOBALS['WEBSERVICES_DUBLIN_CORE_INTERFACE']);
	...
	*/
	...
	$query->From = $database;
	$query->Where = BuildDCWhereStatement($interface->mapping);
	$query->Limit = $limit;
	$query->Offset = $offset;
...
*/
function
BuildDCWhereStatement($mapping)
{
	// only look for fields defined in $DUBLIN_CORE_FIELDS
	global $DUBLIN_CORE_FIELDS;
	global $ALL_REQUEST;
	...
	$queryArray = array();
	foreach($DUBLIN_CORE_FIELDS as $f)
	{
		$dcname = strtolower($f);
		if (isset($ALL_REQUEST[$dcname]) 
				&& $ALL_REQUEST[$dcname] != ""
				&& isset($mapping{$dcname}))
		{
			if (! is_array($mapping{$dcname}))
				WebDie('Invalid Interface Schema');

			$hasterm++;
			foreach ($mapping{$dcname} as $c)
			{
				$qryItem = new QueryItem;
				$qryItem->ColName = $c->Name;
				$qryItem->ColType = $c->Type;
				$qryItem->QueryTerm = $ALL_REQUEST[$dcname];
...
ParseInterfaceDefinitions($definition)
{
	...
	if ($count > 0)
	{
		for ($i = 0; $i < $count; $i++)
		{
			...
			*/
			if (preg_match("/<DCMapping>(.*?)<\/DCMapping>/s", 
				$defs, $matches))
			{
				$s = $matches[1];

				if (preg_match("/<Title>(.*?)<\/Title>/si", $s, $matches))
					$interface->mapping{"title"} = ParseCol($matches[1]);

				if (preg_match("/<Creator>(.*?)<\/Creator>/si", $s, $matches))
					$interface->mapping{"creator"} = ParseCol($matches[1]);

				if (preg_match("/<SubjectKeywords>(.*?)<\/SubjectKeywords>/is", $s, $matches))
					$interface->mapping{"subjectkeywords"} = ParseCol($matches[1]);

				if (preg_match("/<Description>(.*?)<\/Description>/is", $s, $matches))
					$interface->mapping{"description"} = ParseCol($matches[1]);

				if (preg_match("/<Contributor>(.*?)<\/Contributor>/is", $s, $matches))
					$interface->mapping{"contributor"} = ParseCol($matches[1]);

				if (preg_match("/<Date>(.*?)<\/Date>/is", $s, $matches))
					$interface->mapping{"date"} = ParseCol($matches[1]);
