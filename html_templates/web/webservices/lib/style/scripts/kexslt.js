...
//this.url = this.url.replace(/era./,"");
			...
			{
				alert("Error: Transport failed:\n" + err + "\nPossibly mixed host names between javascript and ajax call?");
			}
			
		}
	},

	getResponse: function(debug)
	{
		if (debug)
		...
		else
		{
			...
			var loader = new ajax.Loader(this.xmlUrl,"GET","",
				function() {
					...
					// object 
					xsltObject.xml = this.getResponse(0);
		...
		else
		{
			...
			loader = new ajax.Loader(this.xslUrl,"GET","",
				function() {
					// NB when exectuted, 'this' will equal the ajax.Loader
					// object 
					xsltObject.xsl = this.getResponse(0);
					xsltObject.transform();
				}
			);
			loader.request();
	...
	transform: function()
	{
		...
		{	
			if ( window.ActiveXObject )
			{
				...
				{
					alert("Error transforming with stylesheet\n"+this.xslUrl+"\n"+err);
					return false; 
				}
			}
			else
