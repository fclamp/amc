	...
	*/
	...
	_tableHasData($table, $row=0)
	{
		...
		foreach ($table->Columns as $col)
		{
			...
			for ($num = $min; $num <= $max; $num++)
			{
				...
				{
					$colname = preg_replace('/_tab/', ":$num", $col->ColName);
				}
				elseif(preg_match('/[^:\d]0$/', $col->ColName))
				{
					// A atom table in format FieldID0
					$colname = preg_replace('/0$/', ":$num", $col->ColName);
				}
				elseif(preg_match('/:\d/', $col->ColName))
				{
					$colname = preg_replace('/:\d/', ":$num", $col->ColName);
				}
				else
				{
					if ($num > 1)
						break;  // Not a nested table so must be no more data 
					$colname = $col->ColName;
				}
	...
	_printTable($table)
	{
		if (strtolower(get_class($table)) != 'table')
			return;
		elseIf( ! $this->_tableHasData($table) )
			return;
		...
		for ($row = 1; ; $row++)
		{
			...
			foreach ($table->Columns as $col)
			{
				...
				switch (strtolower(get_class($col)))
				{
					...
					elseif(preg_match('/:\d/', $col->ColName))
					{
						$col->ColName = preg_replace('/:\d/', ":$row", $col->ColName);
					}
