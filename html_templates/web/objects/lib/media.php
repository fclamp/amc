...
class MediaRetriever
{
	...
	MediaRetriever()
	{
		...
		/*
		 * Added to restrict image to a mime format listed in $this->WebFormats
		 */
	...
	ShowImage($irn, $size)
	{
		...
		while (1)
		{
			...
			if ($this->WebImage == 1)
			{
				if ( strpos(strtolower($this->WebFormats), strtolower($attributes->MimeFormat)) === false)
				{
					$i++;
					continue;
				}
		...
		if ($mimetype == "")
		{
			...
			{
			 	if (($maxside > 0 && !empty($availableSizes)) || ($this->WebImage == 1 && !empty($availableSizes))) 
				{
				...
				elseif (($maxside > 0 && empty($availableSizes)) && $r[0]->MulMimeType == "image")	
				{
	...
	{
		if (strpos(strtolower($mimetype), 'tiff') !== false)
			return(1);
		return(0);
	}
	...
	IsImage($mimetype)
	{
		if (strpos(strtolower($mimetype), 'jpeg') !== false)
			return(1);
		if (strpos(strtolower($mimetype), 'gif') !== false)
			return(1);
	...
	function imagewatermarker($file, $mimetype)
	{
		...
		*/
		...
		$logo_resized = imagecreatetruecolor($stamp_x, $stamp_y);
		$background = imagecolorallocate($logo_resized, 0, 0, 0);
		ImageColorTransparent($logo_resized, $background);
		imagecopyresampled($logo_resized, $logoImage, 0, 0, 0, 0, $stamp_x, $stamp_y, ImageSX($logoImage) , ImageSY($logoImage));
		...
		if ($this->WatermarkLogoPos != '')
		{
			...
			{
				$stamp_x = $photoX - ImageSX($logo_resized);
				$stamp_y = 0;
			}
			if (strcasecmp($this->WatermarkLogoPos, 'bl') == 0)
			{
				$stamp_x = 0;
				$stamp_y = $photoY - ImageSY($logo_resized);
			}
			...
			{
				$stamp_x = (($photoX - ImageSX($logo_resized))/2);
				$stamp_y = 0;
			}
			...
			{
				$stamp_x = ($photoX / 2 ) - (ImageSX($logo_resized) / 2 );
				$stamp_y = ($photoY / 2 ) - (ImageSY($logo_resized) / 2 );
			}
		...
		imagecopymerge($photoImage, $logo_resized, $stamp_x, $stamp_y, 0, 0, $photoX, $photoY, $opacity);
		// Output to Browser. 
		ImagePNG($photoImage);
