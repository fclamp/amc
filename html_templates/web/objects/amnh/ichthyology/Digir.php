...
class DigirFactory
{
	function getInstance()
	{
		...
		if ($provider = new AmnhDigir($BACKEND_TYPE,$WEB_ROOT,$WEB_DIR_NAME))
		{
			$provider->configure();
			return $provider;
		}
...
class AmnhDigir extends DigirProvider
{
	...
	var $serviceName = "AmnhDigirProvider";

	function describe()
	{
		return	
			"A AMNH Digir Provider is a AMNH client specific\n".
			"Digir Provider\n\n".
			Parent::describe();
	}
	...
	function testQueryTerms()
	{
		...
		return  "<filter>\n".
			"	<equals>\n".
			"		<darwin:Genus>Barbus</darwin:Genus>\n".
			"	</equals>\n".
			"</filter>\n".
	...
	function configure()
	{
		...
		$this->setSimpleMapping('Class','IdeCurrentClassLocal');
		$this->setSimpleMapping('Order','IdeCurrentOrderLocal');
		$this->setSimpleMapping('Family','IdeCurrentFamilyLocal');
		$this->setSimpleMapping('Genus','IdeCurrentGenusLocal');
		$this->setSimpleMapping('Species','IdeCurrentSpeciesLocal');
		$this->setSimpleMapping('Subspecies','IdeCurrentSubspeciesLocal');
		$this->setSimpleMapping('ScientificName','IdeCurrentScientificNameLocal');
		$this->setSimpleMapping('TypeStatus','IdeCitationTypeStatus_tab');
		$this->setSimpleMapping('IdentifiedBy','DarIdentifiedBy');
		$this->setSimpleMapping('Collector','BioCollectorsLocal');
		$this->setSimpleMapping('Country','BioSiteCountryLocal');
		$this->setSimpleMapping('StateProvince','BioSiteStateLocal');
		$this->setSimpleMapping('County','BioSiteCountyLocal');
		$this->setSimpleMapping('CatalogNumber','CatNumber');
		$this->setSimpleMapping('Locality','BioSitePreciseLocalityLocal');
	...
	function generateValue($field,$record)
	{
		...
		*/
		...
		switch($field)
		{
			...
			case 'Latitude' :
				if (! $record)
					return "BioSiteLatitudeLocal";
				else
					return $this->DegMinSecToDecimal($record['BioSiteLatitudeLocal']);
				break;
			case 'Longitude' :
				if (! $record)
					return "BioSiteLongitudeLocal";
				else
					return $this->DegMinSecToDecimal($record['BioSiteLongitudeLocal']);
				break;
...
if (isset($_REQUEST['test']))
{
	...
	{
		$actualProvider = new AmnhDigir($BACKEND_TYPE,$WEB_ROOT,$WEB_DIR_NAME);
