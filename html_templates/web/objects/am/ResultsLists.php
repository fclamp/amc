...
{
	var $Fields = array(	'WebSummaryData', 'MulMultiMediaRef_tab'
				);	
	var $highlightTerms = "";			
	...
	printNavBasicLinks()
	{
		...
		if ($this->Matches > 0)
		{
			...
			$criteria = $_REQUEST['QueryTerms'];
			if ($criteria == '')
				$criteria =" requested criteria";
			else
				$criteria = '"' . $criteria . '"';

			$summary = "<span> Search results for $criteria " . $this->StartAt . "</span>" .
				" to <span>$upper</span>" .
...
class
AmNarrativeResultsList extends AmStandardResultsList
{
				...
				if (isset($records[$recordnum]) && ($recordnum < $this->LimitPerPage))
				{
					...
					if ($mediaIrn)
					{
						...
						if ($mr->imageIsViewable($mediaIrn))
						{
							...
							$imageLink = "/" . $GLOBALS['WEB_DIR_NAME'] . "/pages/common/imagedisplay.php?irn=$mediaIrn";
							print "\t\t\t\t\t\t<a class='imageLink'  href='$displaylink'>" .
